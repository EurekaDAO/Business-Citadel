...
pub contract EmployeeProfile {
  ...
  pub resource interface Public {
    ...
    pub fun asReadOnly(): EmployeeProfile.ReadOnly
  }
  ...
  pub resource Base: Owner, Public {
    ...
    pub fun asReadOnly(): EmployeeProfile.ReadOnly {
      return EmployeeProfile.ReadOnly(
  ...
  pub struct ReadOnly {
    ...
    init(address: Address?, name: String, email: String, avatar: String, color: String, info: String) {
  ...
  pub fun new(): @EmployeeProfile.Base {
    return <- create Base()
  }
  
  pub fun check(_ address: Address): Bool {
    return getAccount(address)
      .getCapability<&{EmployeeProfile.Public}>(EmployeeProfile.publicPath)
      .check()
  }
  
  pub fun fetch(_ address: Address): &{EmployeeProfile.Public} {
    return getAccount(address)
      .getCapability<&{EmployeeProfile.Public}>(EmployeeProfile.publicPath)
      .borrow()!
  }
  
  pub fun read(_ address: Address): EmployeeProfile.ReadOnly? {
    if let profile = getAccount(address).getCapability<&{EmployeeProfile.Public}>(EmployeeProfile.publicPath).borrow() {
  ...
  pub fun readMultiple(_ addresses: [Address]): {Address: EmployeeProfile.ReadOnly} {
    let profiles: {Address: EmployeeProfile.ReadOnly} = {}
    for address in addresses {
      let profile = EmployeeProfile.read(address)
      if profile != nil {
  ...
  init() {
    self.publicPath = /public/employee_profile
    self.privatePath = /storage/employee_profile
